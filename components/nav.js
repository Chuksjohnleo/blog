...
export default function Nav({path}) {
  ...
  function filterPosts(e){
     //This function returns all the filtered posts to be used
     //as suggestions in the search bar.
     //This is for homepage search
  ...
  function showAllPostsFromSearchResult(){
     //This function returns all the filtered posts when
     // the search button/icon is clicked.
  ...
  function searchResultHandler(postTitle){
    ...
    //has the same title as the post title parameter above 
    // when a suggested title is clicked is clicked.
    const theFilteredPosts = context.posts.filter(post=>{
    ...
    function filterPostsFromDbSearch(e){
      //This function returns all the filtered posts to be used
      //as suggestions in the search bar.
      //This is for homepage search
    const theFilteredPosts = context.posts.filter(post=>{
  ...
  function showAllPostsFromDbSearchResult(){
    //This function returns all the filtered posts when
    // the search button/icon is clicked.
  ...
  function DbSearchResultHandler(postTitle){
    //This function returns only one post which is the post that
    ...
    <>
      <header className={`${headerZindex} sticky backdrop-blur bg-white/70 inset-0`}>
        <div className="flex h-[50px] items-center justify-around border-b">
          <div id="burger">
            <button title="Menu" onClick={openAndCloseSidebar} className="hover:bg-black/60 hover:text-white hover:fill-white p-2">
              <svg
        ...
        <div className={`${searchBarDisplay}`}>
         ...
         <div className={`${searchBarDisplay}`}>
            ...
            <input onInput={(e)=>filterPostsFromDbSearch(e)} className="focus:outline-black/50 w-full text-center" type='text' placeholder="what do you want to search?" />
            <button onClick={showAllPostsFromDbSearchResult} className="border-l bg-black/20 active:stroke-white active:fill-black active:bg-black">
             <svg className="stroke-black fill-transparent active:stroke-white active:fill-black" width="40" height="30" viewBox="0 0 24 24">
               <g id="Interface / Search_Magnifying_Glass">
                <path id="Vector" d="M15 15L21 21M10 17C6.13401 17 3 13.866 3 10C3 6.13401 6.13401 3 10 3C13.866 3 17 6.13401 17 10C17 13.866 13.866 17 10 17Z" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
               </g>
            ...
            <div className='fixed top-[90px] overflow-auto max-h-[60vh] p-2 font-semibold border w-full bg-white'> 
              <GeneralSearchResult searchResultHandler={DbSearchResultHandler} filteredPosts={filteredPosts} /> 
            </div>
         </div>
        :''}
        ...
        <div className="flex justify-around border-t border-b items-center">
          <Link title="Home" className={path==='home'?currentPathClassName:"font-bold p-1 hover:bg-black/60 hover:fill-white hover:text-white"} href="/">
          ...
          <Link title="Write a new post" className={path==='writePost'?currentPathClassName:"font-bold p-1 hover:bg-black/60 hover:text-white hover:fill-white"} href="/write-post">
            <Write />
          </Link>
         
          <Link title="View all posts" className={path==='allPosts'?currentPathClassName:"font-bold p-1 fill-black hover:bg-black/60 hover:text-white hover:fill-white"} href="/posts">
          {/* <svg width="40" height="30" viewBox="0 0 24 24">
           ...
           <svg width="40" height="30" viewBox="-5.5 0 61.432 61.432" xmlns="http://www.w3.org/2000/svg">
            <g data-name="Group 56" transform="translate(-447.948 -926.141)">
             <path data-name="Path 42" d="M497.139,938.044h-5.417v-5a1.25,1.25,0,0,0-1.25-1.25h-4.746v-4.4a1.25,1.25,0,0,0-1.25-1.25H449.2a1.249,1.249,0,0,0-1.25,1.25v47.028a1.25,1.25,0,0,0,1.25,1.25h4.747v4.4a1.25,1.25,0,0,0,1.25,1.25h5.416v5a1.25,1.25,0,0,0,1.25,1.25h35.278a1.25,1.25,0,0,0,1.25-1.25V939.294A1.25,1.25,0,0,0,497.139,938.044Zm-46.691,35.125V928.641h32.778v3.15H455.2a1.25,1.25,0,0,0-1.25,1.25v40.128Zm6,5.651V934.291h32.777v3.753H461.861a1.25,1.25,0,0,0-1.25,1.25V978.82Zm39.444,6.253H463.111V940.544h32.778Z" />
             <path data-name="Path 43" d="M466.993,968.634a1.25,1.25,0,0,0,0,2.5h25.014a1.25,1.25,0,0,0,0-2.5Z" />
             <path data-name="Path 44" d="M492.007,973.444H466.993a1.25,1.25,0,0,0,0,2.5h25.014a1.25,1.25,0,0,0,0-2.5Z" />
             <path data-name="Path 45" d="M492.007,978.256H466.993a1.25,1.25,0,0,0,0,2.5h25.014a1.25,1.25,0,0,0,0-2.5Z" />
            <rect data-name="Rectangle 34" width="25.015" height="20.976" transform="translate(466.993 943.975)" />
            </g>
           </svg>
          </Link>
          {path==='home' || path==='allPosts' ?
          <>{searchBarDisplay==='hidden'?
          <button title="Search" onClick={openSearchbar} className="hover:bg-black/60 font-bold p-1 stroke-black hover:stroke-white fill-white hover:fill-transparent hover:text-white">
            <svg width="40" height="30" viewBox="0 0 24 24">
              <g id="Interface / Search_Magnifying_Glass">
               <path id="Vector" d="M15 15L21 21M10 17C6.13401 17 3 13.866 3 10C3 6.13401 6.13401 3 10 3C13.866 3 17 6.13401 17 10C17 13.866 13.866 17 10 17Z" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
              </g>
